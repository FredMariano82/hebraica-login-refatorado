  ...
  <div className="overflow-x-auto border rounded-lg max-h-[70vh] overflow-y-auto relative">
    <Table>
      <TableHeader className="sticky top-0 bg-slate-100 z-10 shadow-sm">
        <TableRow>
          {colunasVisiveis["prestador"] && <TableHead className="font-bold text-slate-700">Prestador</TableHead>}
          {colunasVisiveis["documento"] && <TableHead className="font-bold text-slate-700">Documento</TableHead>}
          {colunasVisiveis["dataInicial"] && <TableHead className="font-bold text-slate-700">Data Inicial</TableHead>}
          {colunasVisiveis["dataFinal"] && <TableHead className="font-bold text-slate-700">Data Final</TableHead>}
          {colunasVisiveis["liberacao"] && <TableHead className="font-bold text-slate-700">Liberação</TableHead>}
          {colunasVisiveis["checagem"] && <TableHead className="text-center font-bold text-slate-700">Checagem</TableHead>}
          {colunasVisiveis["validaAte"] && <TableHead className="font-bold text-slate-700">Válida até</TableHead>}
          {colunasVisiveis["acoes"] && <TableHead className="font-bold text-slate-700">Ações</TableHead>}
          {colunasVisiveis["justificativa"] && <TableHead className="font-bold text-slate-700 w-[200px]">Justificativa</TableHead>}
        </TableRow>
      </TableHeader>
      <TableBody>
        {dadosFiltrados.map(({ solicitacao, prestador, prioridade }, index) => {
          ...
          return (
            <TableRow
              key={`${solicitacao.id}-${prestador.id}-${index}`}
              className={`hover:bg-slate-50 ${prioridade <= 3 ? "bg-red-50 hover:bg-red-100" : ""
                }`}
            >
              {/* ... cells ... */}
            </TableRow>
